package org.twightlight.hlootchest.supports.v1_17_R1;
...
import org.twightlight.hlootchest.supports.v1_17_R1.listeners.ClickEvent;
import org.twightlight.hlootchest.supports.v1_17_R1.utilities.NMSUtil;
...
import static net.minecraft.network.protocol.game.PacketPlayOutGameStateChange.d;
...
public class Main extends NMSHandler {
    ...
    public void registerButtonClick(Player player) {
        ...
        nmsPlayer.b = new ClickEvent(nmsPlayer.b.a, nmsPlayer);
    }
    ...
    public static void rotate(net.minecraft.world.entity.decoration.EntityArmorStand armorStand, TConfigManager config, String path) {
        if (config.getYml().getConfigurationSection(path + ".rotations") != null) {
            ...
            for (String s : rotations) {
                ...
                Vector3f rotation = Main.stringToVector3f(rotationString);
    ...
    public void setFakeGameMode(Player p, GameMode gamemode) {
        ...
        switch (gamemode) {
            case SURVIVAL:
                packet = new PacketPlayOutGameStateChange(d, 0);
                break;
            case CREATIVE:
                packet = new PacketPlayOutGameStateChange(d, 1);
                break;
            case ADVENTURE:
                packet = new PacketPlayOutGameStateChange(d, 2);
                break;
            case SPECTATOR:
                packet = new PacketPlayOutGameStateChange(d, 3);
                break;
        }
